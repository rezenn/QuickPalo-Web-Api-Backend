...
export class UserService {
  ...
  async generateStreamToken(userId: string) {
    ...
    // Upsert user to Stream with complete info
  ...
  async sendMessageToOrganization(
    ...
    try {
      ...
      await streamServerClient.upsertUsers([
        {
          ...
          image: sender.profilePicture
            ? `${process.env.BASE_URL || "http://localhost:5050"}/uploads/profile/${sender.profilePicture}`
            : undefined,
        },
        {
          id: orgUserIdStr,
          ...
          image: organizationUser.profilePicture
            ? `${process.env.BASE_URL || "http://localhost:5050"}/uploads/profile/${organizationUser.profilePicture}`
            : undefined,
        },
      ]);
      ...
      try {
        ...
        await channel.watch(); // This will throw if channel doesn't exist
      } catch (error) {
        // Channel doesn't exist, create it
        channel = streamServerClient.channel("messaging", channelId, {
          // name: `${sender.fullName} & ${organizationUser.fullName}`,
          members: [senderIdStr, orgUserIdStr],
          created_by_id: senderIdStr, // Use created_by_id instead of created_by
        });
